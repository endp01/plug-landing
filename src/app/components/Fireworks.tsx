...
export default function Fireworks({ enabled = false }: { enabled: boolean }) {
  ...
  useEffect(() => { 
    ...
    const getAnimationSettings = (originXA: number, originXB: number, particleCount: number) => {
      return {
        startVelocity: 30,
        spread: 360,
        ticks: 60,
  ...
  return (
      <ReactCanvasConfetti 
        ...
        className="h-screen w-screen fixed top-0 left-0 z-[50] pointer-events-none"
      />
  );
}
